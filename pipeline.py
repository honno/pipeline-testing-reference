...
def download_latest_cereals() -> pd.DataFrame:
    ...
    assert isinstance(df, pd.DataFrame)  # for mypy
    return df
...
def preprocess_cereals(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "name" not in df.columns:
        if "brand" in df.columns:
            renamed_df = df.rename({"brand": "name"}, axis=1)
            assert isinstance(renamed_df, pd.DataFrame)  # for mypy
            df = renamed_df
        else:
...
def find_highest_protein_cereal(df: pd.DataFrame) -> str:
    ...
    assert isinstance(sorted_df, pd.DataFrame)  # for mypy
